...
let supa = null;
try { if (SUPA_URL && SUPA_KEY && window.supabase) { supa = window.supabase.createClient(SUPA_URL, SUPA_KEY); } } catch {}
...
const supaUrlEl = document.getElementById('supaUrl'); const supaKeyEl = document.getElementById('supaKey'); const saveSupaBtn = document.getElementById('saveSupa');
if (supaUrlEl) supaUrlEl.value = SUPA_URL; if (supaKeyEl) supaKeyEl.value = SUPA_KEY;
saveSupaBtn?.addEventListener('click', ()=>{ SUPA_URL = (supaUrlEl?.value||'').trim(); SUPA_KEY = (supaKeyEl?.value||'').trim(); localStorage.setItem('supaUrl', SUPA_URL); localStorage.setItem('supaKey', SUPA_KEY); try { if (SUPA_URL && SUPA_KEY && window.supabase) { supa = window.supabase.createClient(SUPA_URL, SUPA_KEY); showToast('Supabase сохранён'); } else { showToast('Укажите URL и anon key Supabase'); } } catch(e){ showToast(e.message);} });
